...
| `DROP_INPUT_GRAPH`          | If `true`, the task's input graph is `DROP`ped after a successful transformation. Set to `false` when the input graph is a shared/persistent graph (e.g. the app's public graph) that must not be wiped. | `true`                                    |
