...
export async function loadTask(subject) {
  const queryTask = `
   ${PREFIXES}
   SELECT DISTINCT ?graph ?task ?inputGraph ?bestuurseenheid ?id ?job ?created ?modified ?status ?index ?operation ?error WHERE {
     GRAPH ?graph {
      ...
      ?inputContainer task:hasGraph ?inputGraph.
      OPTIONAL { ?task ext:hasResource ?bestuurseenheid. }
