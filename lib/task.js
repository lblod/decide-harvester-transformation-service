...
export async function loadTask(subject) {
  ...
  task.containers = await loadTaskContainers(task);

  return task;
}

async function loadTaskContainers(task) {
  const queryContainers = `
   ${PREFIXES}
   SELECT DISTINCT ?inputContainer ?inputGraph ?bestuurseenheid WHERE {
     GRAPH ${sparqlEscapeUri(task.graph)} {
       ${sparqlEscapeUri(task.task)} task:inputContainer ?inputContainer.
       OPTIONAL { ?inputContainer task:hasGraph ?inputGraph. }
       OPTIONAL { ?inputContainer task:hasResource ?bestuurseenheid. }
     }
   }
  `;
  return parseResult(await query(queryContainers, connectionOptions));
}
