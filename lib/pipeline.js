...
export async function run(deltaEntry) {
  ...
  try {
    if (!task.containers.length) {
      throw new Error("Task does not have any input containers.");
    }

    const incompleteContainers = task.containers.filter((c) => !c.inputGraph || !c.bestuurseenheid);
    if (incompleteContainers.length) {
      throw new Error(
        `Task has input container(s) missing task:hasGraph and/or task:hasResource: ${incompleteContainers
          .map((c) => c.inputContainer)
          .join(", ")}`,
      );
    }
    ...
    for (const container of task.containers) {
      const resourceGraph = container.inputGraph;
      const normalizedBestuurseenheid = container.bestuurseenheid.replace(/^https:\/\//, "http://");
    ...
    for (const container of task.containers) {
      await dropGraph(container.inputGraph);
    }
  } catch (e) {
    console.error(e);
    if (task) {
