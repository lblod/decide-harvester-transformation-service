...
export async function run(deltaEntry) {
  ...
  if (resourceGraph) {
    try {
      await update(
        `DROP SILENT GRAPH ${sparqlEscapeUri(resourceGraph)}`,
        {},
        { sparqlEndpoint: DIRECT_SPARQL_ENDPOINT, mayRetry: true },
      );
    } catch (e) {
...
async function countProperties(countQuery) {
  const response = await query(
    ...
    {},
    {
      sparqlEndpoint: BYPASS_MU_SPARQL_ENDPOINT ? DIRECT_SPARQL_ENDPOINT : MU_SPARQL_ENDPOINT,
      mayRetry: true,
    },
  );
  await sleep();
